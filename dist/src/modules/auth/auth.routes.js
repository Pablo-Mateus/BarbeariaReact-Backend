...
import { registerSchema, loginSchema, resetPasswordSchema, changePasswordSchema, confirmEmailSchema, } from './auth.validation.js';
const router = Router();
...
router.get('/confirm-email', validate({ query: confirmEmailSchema }), authController.confirmEmail);
router.post('/reset-password', authRateLimiter, validate({ body: resetPasswordSchema }), authController.resetPassword);
router.post('/change-password', authRateLimiter, validate({ body: changePasswordSchema }), authController.changePassword);
// Protected route
